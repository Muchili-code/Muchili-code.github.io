...
| **步骤**                 | **运行什么代码**                                      | **预期结果**                                                 |
| ------------------------ | ----------------------------------------------------- | ------------------------------------------------------------ |
| **1. 进入仓库 A**        | `cd "D:\Program Files\Github\Muchili-code.github.io"` | 当前工作目录切换到 A 仓库的根目录。                          |
| **拉取子仓库的最新更新** | `git submodule update --reomte`                       | 告诉我们已经有`docs/notes`更新了                             |
| **2. 查看状态**          | `git status`                                          | 您会看到类似这样的提示：`modified: docs/notes (new commits)`。这表明 Git 已经检测到子模块指向了一个新的提交。 |
| **3. 暂存引用更新**      | `git add docs/notes`                                  | 这一步不是添加文件内容，而是添加子模块 **新 Commit ID** 的引用记录。 |
| **4. 提交引用更新**      | `git commit -m "更新子模块 notes 至最新版本"`         | A 仓库将这个新的子模块 Commit ID 记录在自己的提交历史中。    |
| **5. 推送 A 仓库**       | `git push origin main` (或您的主分支名称)             | 将 A 仓库关于子模块引用的更新推送到 A 的远程 GitHub 仓库。   |
